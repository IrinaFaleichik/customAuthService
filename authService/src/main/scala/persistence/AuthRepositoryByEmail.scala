...
final class AuthRepositoryByEmail(ctx: DBContext) extends AuthRepository[EmailIdentity]:
  ...
  private def hashPassword(password: String): ZIO[HashingUtilsService, Throwable, HashedPassword] = HashingUtilsService.fromPlainText(password)

  private def verifyPassword(dbPassword: String, hash: HashedPassword): Boolean = hash.verifyHashed(dbPassword)

  override def authenticate(identity: EmailIdentity): ZIO[HashingUtilsService, Throwable, AuthUserDto] =
    for
